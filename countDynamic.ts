...
let dynamicTable = document.querySelector(
  ".dynamic-table-values"
) as HTMLElement;

function resetCountDynamic(main: string, sub: string) {
  let table = document.querySelector(".dynamic-table") as HTMLElement;
  table?.remove();
  ...
  for (let i = 0; i <= subStringCountDynamic.length; ++i) {
    lookup[0][i] = 0;
    let cell = document.querySelector(`#cell-${i + 2}-2`)! as HTMLElement;
    cell.innerText = "0";
  ...
  for (let i = 0; i <= mainStringCountDynamic.length; ++i) {
    lookup[i][0] = 1;
    let cell = document.querySelector(`#cell-2-${i + 2}`)! as HTMLElement;
    cell.innerText = "1";
    cell.className = "cell first-cell";
  }
...
let highlightColumn: HTMLElement | null = null;
let highlightRow: HTMLElement | null = null;
let highlightedResultCell: HTMLElement | null = null;
...
function setDynamicGridHighlight() {
  ...
  let row = document.querySelector(`#cell-${i2Dynamic + 2}-1`)! as HTMLElement;
...
function setDynamicCellHighlight(active: boolean = true) {
...
function countDynamic(): any {
  ...
  if (
    mainStringCountDynamic.charAt(i1Dynamic - 1) ==
